...
/**
 * Format rec object with customs properties
 *
 * @param rec {log record}
 * @param msgArgs {args}. Check if an arg has isMeta true and add it as a property
 */
Logger.prototype._formatMeta = function (rec, msgArgs) {
    try {
        var metaIndexesToDelete = [];
        
        // Put meta properties on rec 
        for(var i = 0; i < msgArgs.length; i++) {
            if(typeof(msgArgs[i])==='object' && msgArgs[i].isMeta) {
                rec.meta = {};
                delete msgArgs[i].isMeta;
                for(var metaProp in msgArgs[i]) {
                    rec.meta[metaProp] = msgArgs[i][metaProp];
                }
                delete msgArgs[i];
                metaIndexesToDelete.push(i);
            }
        }
        // Remove meta objects
        for(var index of metaIndexesToDelete) {
            msgArgs.splice(index, 1);
        }
    }
    catch(ex) {}
}
